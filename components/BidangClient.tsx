        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {bidangList.map((bidang) => (
              <div
                key={bidang.key}
                className="group bg-white dark:bg-gray-800 p-6 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between"
              >
                <div>
                  <div className="w-12 h-12 mb-4 rounded-xl bg-red-50 dark:bg-red-900/30 text-brand-red flex items-center justify-center group-hover:bg-brand-red group-hover:text-white transition-colors duration-300">
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      {bidang.icon}
                    </svg>
                  </div>
                  <h3 className="font-bold text-lg text-brand-dark dark:text-white mb-2">{bidang.title}</h3>
                  <p className="text-gray-600 dark:text-gray-300 text-sm mb-4">{bidang.desc}</p>
                </div>
                <button
                  type="button"
